...
import { format, differenceInDays, differenceInMinutes, differenceInSeconds ,differenceInHours} from 'date-fns';
...
//   const userData = useUser();
 ...
 const  getquizresponsebyId = async () => {
  axios.post(baseUrl + QUIZ.getQuizbyresponseId ,{quizid : id} ).then((response) => {
  ...
  const whenSubmitted = async (answers) => {
    ...
    responseData.quizAttend.startedAt = Date.now();
    ...
    if (responseData.quizAttend.average > responseData.quizAttend.passPrecentage) {
          ...
          const { error: insertError } = await supabase
            ...
            .insert([{
              ...
              student_info: [
                {
                  student_id: currentuserdata?.id,
                  student_firstname: currentuserdata?.firstname +" "+currentuserdata?.lastname,
                  student_email: currentuserdata?.email,
                  student_score: pointgat,
                }
              ],
...
const pages = [
  <LoginBeforeAttend key="login-before-attend" setIndex={setIndex} LogIn={LogIn} />,
  <AttendForm key="attend-form" whenFormSubmitted={whenFormSubmitted} />,
  <div key="quiz-page" className={` ${quizData && (quizData.themeId == "1" ? "bg-yellow-500": "bg-red-500")}  relative flex justify-center items-center`}>
    {/* {console.log(attemptCompleted)} */}
    {/* {console.log(responseData.numAttempt)} */}
    {quizData && (quizData.themeId == "1" ? <div className="absolute -left-[50vw] -top-1/2 w-screen   z-10 ">
    <svg preserveAspectRatio="xMidYMid slice" viewBox="10 10 80 80" width="100vw" height="100vh">
    <defs>
      <style>
        {`
          @keyframes rotate {
    ...
    <path fill="#9b5de5" className="outTop" d="M37-5C25.1-14.7,5.7-19.1-9.2-10-28.5,1.8-32.7,31.1-19.8,49c15.5,21.5,52.6,22,67.2,2.3C59.4,35,53.7,8.5,37-5Z"/>
    <path fill="#f15bb5" className="inTop" d="M20.6,4.1C11.6,1.5-1.9,2.5-8,11.2-16.3,23.1-8.2,45.6,7.4,50S42.1,38.9,41,24.5C40.2,14.1,29.4,6.6,20.6,4.1Z"/>
    <path fill="#00bbf9" className="outBottom" d="M105.9,48.6c-12.4-8.2-29.3-4.8-39.4.8-23.4,12.8-37.7,51.9-19.1,74.1s63.9,15.3,76-5.6c7.6-13.3,1.8-31.1-2.3-43.8C117.6,63.3,114.7,54.3,105.9,48.6Z"/>
    <path fill="#00f5d4" className="inBottom" d="M102,67.1c-9.6-6.1-22-3.1-29.5,2-15.4,10.7-19.6,37.5-7.6,47.8s35.9,3.9,44.5-12.5C115.5,92.6,113.9,74.6,102,67.1Z"/>
  </svg>
    </div> : quizData.themeId == "2" ? <div className="animated2 w-full absolute  w-screen h-screen  -translate-x-1/2 -translate-y-1/2 top-[125%] mt-[50vh] -left-2/4  z-10  " ></div> : <div className="container w-full absolute  w-screen h-screen  -translate-x-1/2 -translate-y-1/2 top-[125%] mt-[50vh] -left-2/4  z-10  ">
  <div className="lines">
   ...
   <div className="z-20 absolute -left-[40vw] -top-[100%] bg-zinc-500/50 max-h-screen overflow-y-auto rounded-bl-3xl rounded-br-3xl p-3 backdrop-blur-sm "> 
    {responseData && quizData  || quizData?.isActive ? 
    ((quizData.timeline == true ? (
      quizData.startdate > Date.now() || quizData.enddate < Date.now() ? ((attemptCompleted !== responseData.numAttempt ? (
        quizData.isActive ? (
          ...
          <div className="flex w-full flex-col   justify-center items-center min-h-screen sm:w-[80vw]">
            <div className="flex flex-col  h-1/2 w-1/2 justify-center items-center bg-zinc-200/50  backdrop-blur-xl rounded-3xl h-[60vh] text-center p-3 text-center text-lg sm:text-xl">
