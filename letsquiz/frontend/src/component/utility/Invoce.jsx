...
const Invoice = ({ currentSub }) => {
  ...
  useEffect(() => {
    if (currentSub) {
      taxes.forEach((tax) => {
        handleTaxData({
          taxname: tax.taxName,
          taxamount: (tax.taxPercentage * currentSub?.amount) / 100,
  ...
  const amountbeforetax = currentSub?.amount;

  console.log(amountbeforetax);

  const rate = useMemo(() => {
    return (
      currentSub?.amount + (totaltaxespersentage * currentSub?.amount) / 100
    );
  }, [totaltaxespersentage, currentSub]);
  ...
  const getalltaxes = () => {
    axios
      ...
      .then((res) => {
        ...
        const gstTaxes = res.data.filter(
          (tax) => tax.taxType === "GST"
        );
        settaxes(gstTaxes);
  ...
  const totalAmount =
    ...
    apllyalldiscount -
    discountamount +
    (totaltaxespersentage * currentSub?.amount) / 100;

  const gstTaxes = totaltaxdata.filter(
    (tax) => tax.taxname === "CGST" || tax.taxname === "SGST"
  );
  console.log(gstTaxes);
      ...
      .then((res) => {
        axios
          .post(`${baseUrl}transaction/`, {
            ...
            subcriptionname: currentSub?.name,
            paymentMethod: paymentMethod,
            status: "success",
            persentepaidtoowner: 100,
            username:
    ...
    <div className="flex h-fit w-screen flex-wrap justify-start rounded-md bg-white p-6 shadow-md">
      <div
        className={`flex  flex-col items-start justify-start px-4 ${
          currentSub?.amount > 0 ? "w-1/2" : "w-full"
        }`}
      >
        {/* Contact Details */}
        ...
        <div className="mb-6 mt-8 w-full md:mb-0 md:w-full">
          ...
          <div className="space-y-2">
            ...
            <div className="flex  items-start justify-between">
              ...
              <div className="flex flex-col justify-start gap-1">
                {taxes.map((item, index) => (
                  <div key={index} className="flex w-full justify-between">
                    <span>
                      ₹{(item.taxPercentage * currentSub?.amount) / 100}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {currentSub?.amount > 0 && apllyalldiscount && (
